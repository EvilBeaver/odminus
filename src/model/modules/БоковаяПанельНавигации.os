...
Функция СформироватьНавигацию(Знач Маршрутизатор) Экспорт
	...
	Иначе
		...
		ДобавитьСсылкуКластера(Описание, Кластер, Маршрутизатор, "infobases","Информационные базы", ПутьТекущегоЗапроса);
		ДобавитьСсылкуКластера(Описание, Кластер, Маршрутизатор, "processes","Рабочие процессы", ПутьТекущегоЗапроса);
		ДобавитьСсылкуКластера(Описание, Кластер, Маршрутизатор, "servers","Рабочие серверы", ПутьТекущегоЗапроса);
		ДобавитьСсылкуКластера(Описание, Кластер, Маршрутизатор, "sessions","Сеансы", ПутьТекущегоЗапроса);
		ДобавитьСсылкуКластера(Описание, Кластер, Маршрутизатор, "connections","Соединения", ПутьТекущегоЗапроса);
	КонецЕсли;
